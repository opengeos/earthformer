...
from earthformer.utils.checkpoint import (
    pl_ckpt_to_pytorch_state_dict,
    s3_download_pretrained_ckpt,
)
from earthformer.utils.layout import layout_to_in_out_slice
from earthformer.cuboid_transformer.cuboid_transformer_unet_dec import (
    CuboidTransformerAuxModel,
)
from earthformer.datasets.earthnet.earthnet_dataloader import (
    EarthNet2021LightningDataModule,
    get_EarthNet2021_dataloaders,
)
from earthformer.datasets.earthnet.earthnet_scores import (
    EarthNet2021ScoreUpdateWithoutCompute,
)
...
class CuboidEarthNet2021PLModule(pl.LightningModule):
    def __init__(self, total_num_steps: int, oc_file: str = None, save_dir: str = None):
        ...
        if isinstance(model_cfg["cross_self_pattern"], str):
            ...
            dec_self_attn_patterns = OmegaConf.to_container(
                model_cfg["cross_self_pattern"]
            )
        ...
        self.torch_nn_module = CuboidTransformerAuxModel(
            ...
            initial_downsample_stack_conv_num_layers=model_cfg[
                "initial_downsample_stack_conv_num_layers"
            ],
            initial_downsample_stack_conv_dim_list=model_cfg[
                "initial_downsample_stack_conv_dim_list"
            ],
            initial_downsample_stack_conv_downscale_list=model_cfg[
                "initial_downsample_stack_conv_downscale_list"
            ],
            initial_downsample_stack_conv_num_conv_list=model_cfg[
        ...
        self.valid_ens = EarthNet2021ScoreUpdateWithoutCompute(
            layout=self.layout, eps=1e-4
        )
        self.test_mse = torchmetrics.MeanSquaredError()
        self.test_mae = torchmetrics.MeanAbsoluteError()
        self.test_ens = EarthNet2021ScoreUpdateWithoutCompute(
            layout=self.layout, eps=1e-4
        )
    ...
    def get_model_config(cls):
        ...
        # different from CuboidTransformerModel, no arg `dec_use_first_self_attn=False`
        cfg.auxiliary_channels = (
            7  # 5 from mesodynamic, 1 from highresstatic, 1 from mesostatic
        )
    ...
    def get_optim_config():
        ...
        cfg.lr_scheduler_mode = (
            "cosine"  # Can be strings like 'linear', 'cosine', 'platue'
        )
    ...
    def get_trainer_config():
        ...
        cfg.log_step_ratio = (
            0.001  # Logging every 1% of the total training steps per epoch
        )
        cfg.precision = 32
        return cfg
    ...
    def set_trainer_kwargs(self, **kwargs):
        ...
        ret = dict(
            ...
            accelerator="gpu",
            # strategy="ddp",
            strategy=ApexDDPStrategy(
                find_unused_parameters=False, delay_allreduce=True
            ),
            # optimization
    ...
    def forward(self, batch):
        ...
        mesodynamic_interp = F.interpolate(
            mesodynamic_interp,
            size=(
                self.oc.layout.in_len + self.oc.layout.out_len,
        ...
        highresstatic_interp = F.interpolate(
            highresstatic_interp,
            size=(
                self.oc.layout.in_len + self.oc.layout.out_len,
                self.oc.layout.img_height,
                self.oc.layout.img_width,
        ...
        mesostatic_interp = F.interpolate(
            mesostatic_interp,
            size=(
                self.oc.layout.in_len + self.oc.layout.out_len,
                self.oc.layout.img_height,
        ...
        aux_data = torch.cat(
            (highresstatic_interp, mesodynamic_interp, mesostatic_interp), dim=1
        )
        aux_data = rearrange(aux_data, "b c t h w -> b t h w c")

        pred_seq = self.torch_nn_module(
            in_seq, aux_data[self.in_slice], aux_data[self.out_slice]
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log(
            'valid_loss_epoch', valid_loss, prog_bar=True, on_step=False, on_epoch=True
        )
        self.log(
            'valid_mse_epoch', valid_mse, prog_bar=True, on_step=False, on_epoch=True
        )
        self.log(
            'valid_mae_epoch', valid_mae, prog_bar=True, on_step=False, on_epoch=True
        )
        self.log(
            'valid_MAD_epoch',
            valid_ens_dict["MAD"],
        ...
        self.log(
            'valid_EarthNetScore_epoch',
            valid_ens_dict["EarthNetScore"],
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log(
            f'{prefix}_test_EarthNetScore_epoch',
            test_ens_dict["EarthNetScore"],
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
...
def main():
    ...
    if args.pretrained:
        args.cfg = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "earthformer_earthnet_v1.yaml")
        )
    if args.cfg is not None:
        ...
        dataset_cfg = OmegaConf.to_object(
            CuboidEarthNet2021PLModule.get_dataset_config()
        )
        micro_batch_size = 1
    ...
    if args.pretrained:
        ...
        if not os.path.exists(
            os.path.join(pretrained_checkpoints_dir, pretrained_ckpt_name)
        ):
            s3_download_pretrained_ckpt(
                ckpt_name=pretrained_ckpt_name,
        ...
        for test_dataloader in dataloader_dict["test_dataloader"]:
            trainer.test(
                model=pl_module, dataloaders=test_dataloader, ckpt_path=ckpt_path
            )
    else:
        ...
        torch.save(
            state_dict,
            os.path.join(pl_module.save_dir, "checkpoints", pytorch_state_dict_name),
        )
        pl_module.reset_test_epoch_count()
        for test_dataloader in dataloader_dict["test_dataloader"]:
            trainer.test(
